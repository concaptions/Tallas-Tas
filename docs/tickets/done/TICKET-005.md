...
- Payload types omit `brand_id`: `insert` takes `Omit<PgInsertValue<T>, 'brandId'>` (one row or an
  ...
  payload stores A" exercises. (Superseded in round 4, below: both payload types derive from
  `T['$inferInsert']`.)
...
## Round 4 (integrator, stage 2)

Adversary required change (round 3): `Omit<PgInsertValue<T>, 'brandId'>` dropped every optional column
for a generic `T` (`keyof ScopedInsertValue<typeof brandAssignments>` was `"userId" | "role"`), so a
scoped insert with `createdBy`, `deletedAt` or `id` in an object literal failed with TS2353 although
plain Drizzle accepts them. Fix, in `tenancy.ts` only: both payload types derive from the table's insert
model, `ScopedInsertValue<T> = Omit<T['$inferInsert'], 'brandId'>` and
`ScopedUpdateSet<T> = Partial<Omit<T['$inferInsert'], 'brandId'>>`; the `PgUpdateBuilder` and
`PgQueryResultHKT` imports go. The old update type (through `PgUpdateSetSource`, all-optional) did not
collapse, but both now derive from the one model as prescribed. Runtime unchanged: `insert` and `update`
still merge `{ ...payload, brandId }` behind the same assertions, `softDelete` is untouched, and every
round-2 test passes as before. Consequence: a payload value can no longer be an `SQL` fragment (the old
types allowed `SQL | Placeholder` per column); no caller needs one, `softDelete` covers the known case
(`deleted_at = now()`), and a scope that needs it widens the value type then.

Tests (`tenancy.test.ts`): compile-time, `keyof ScopedInsertValue<typeof brandAssignments>` and
`keyof ScopedUpdateSet<...>` equal `Exclude<keyof NewBrandAssignment, 'brandId'>`, neither has `brandId`,
object literals with `id`, `createdBy` and `deletedAt` compile on `insert` (single and batch) and
`update` (each failed with TS2353 on the round-3 code, checked with a scratch probe, deleted), and
`brandId` in a literal is `@ts-expect-error` on all three. Runtime, one test: a scoped insert with `id`
and `createdBy` stores them with `brand_id = A`, a scoped update with `createdBy` and `deletedAt` stores
both and the row leaves `select`.

Stuck report: `TICKET-005.stuck-report.md` was never tracked on `main` (it exists on `wip/ticket-005`
only) and "Round history" below already carries its content, so there is nothing to `git rm`. Non-test
LOC, counted as in round 3: **425 raw / 284 strict** (`tenancy.ts` 142/75, `vitest.config.ts` 3/1, every
other file as in round 2). No `docs/decisions.md` entry (no dependency, no deviation, no D-number named)
and no gated criteria (D-008).

## Round history

Three build → QA → review rounds ran without a full approval, so the loop stopped under the engineering
brief's stuck rule (§7) with the work left uncommitted in the tree; the human approved one more round for
the single item left. This section folds in the stuck-rule report that recorded the stop (2026-09-16).

| Round | QA                                                                                                                    | Product reviewer                           | Architecture reviewer                                                                                             | Adversary                                                                                                                                                                                  |
| ----- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1     | pass                                                                                                                  | approve                                    | request changes: non-test diff 309 lines; `seed.ts` had grown an upsert-on-natural-key layer criterion 4 never asked for | request changes: three escapes from `withBrand` (`$dynamic().where()` replaced the scope; a raw top-level `OR` filter was not parenthesised; `onConflictDoUpdate({ set: { brandId } })` moved a row) |
| 2     | fail: `pnpm test` timed out on PGlite boot (5 s default) under machine load 14-19 on 8 cores; passed on the middle run | request changes: raise the PGlite timeout | same                                                                                                              | approve                                                                                                                                                                                    |
| 3     | pass (3/3 runs)                                                                                                       | approve                                    | approve                                                                                                           | request changes: `ScopedInsertValue<T> = Omit<PgInsertValue<T>, 'brandId'>` drops every optional column, so `createdBy`, `deletedAt` and `id` fail with TS2353 on the scoped insert |

The round-1 adversary findings were fixed in round 2 with regression tests ("Round 2 (integrator, stage 2)"
above). The round-2 QA failure was environmental and fixed in round 3 by `testTimeout: 30_000` ("Round 3
(schema, stage 1)" above).

Round 4, stage 1 (schema): no stage-1 file changed. Re-verified on this run: `pnpm install`,
`pnpm typecheck`, `pnpm lint`, `pnpm test` (47/47) and `drizzle-kit generate` ("No schema changes, nothing
to migrate") all pass. Round 4, stage 2 (integrator): the round-3 adversary item, in `tenancy.ts` only.
Derive the payload types from the table's insert model, `Omit<T['$inferInsert'], 'brandId'>` for inserts
and `Partial<Omit<T['$inferInsert'], 'brandId'>>` for updates, and add a compile-time test that
`createdBy`, `deletedAt` and `id` are accepted by insert and update while `brandId` is still rejected;
runtime behaviour (brand_id forced to the scope) unchanged.

Round 5, stage 1 (schema): no stage-1 file changed. Re-verified on this run: `pnpm install` (already up to
date), `pnpm typecheck`, `pnpm lint` (zero warnings), `pnpm test` (49/49, 7 files, 13.8 s at load 5.9) and
`drizzle-kit generate` ("No schema changes, nothing to migrate") all pass. The stuck-rule report
`docs/tickets/in-progress/TICKET-005.stuck-report.md` is tracked on `wip/ticket-005` only and absent from
`main`'s tree and index, so there is nothing to `git rm`; this section is its fold. The round-4 adversary
item (a raw `sql` fragment that closes `scopeOf`'s wrapping parenthesis, `true) or (true`, escapes the
scope on select, update and softDelete) sits in `packages/db/src/tenancy.ts` and is stage 2's.

Round 6 (schema): closed. The round-4 adversary item is in the tree as prescribed —
`ScopedInsertValue<T> = Omit<T['$inferInsert'], 'brandId'>` and
`ScopedUpdateSet<T> = Partial<Omit<T['$inferInsert'], 'brandId'>>` in `packages/db/src/tenancy.ts`,
with the compile-time coverage in `tenancy.test.ts` ("accepts every optional column on insert and
update, but never brand_id": `keyof` equals `Exclude<keyof NewBrandAssignment, 'brandId'>`, object
literals carrying `id` / `createdBy` / `deletedAt` compile on insert, batch insert and update, and
`brandId` is `@ts-expect-error` on all three). Runtime guarantees unchanged: `brand_id` forced to the
scope, the sealed surfaces expose no `where`, `$dynamic` or `onConflict*`, and the caller's filter is
parenthesised by `contained`. Verified on this run: `pnpm --filter @tas/db exec tsc --noEmit` clean and
`pnpm --filter @tas/db exec vitest run` 32/32 (4 files), the extra tests coming from TICKET-005's
successor work in the same package. The stuck-rule report exists only in commit `b9ec6aa` on
`wip/ticket-005`, never in `main`'s tree or index, so there is nothing to `git rm`; the "Round history"
table above is its fold. Ticket moved to `docs/tickets/done/` and the backlog status cell set to done.
